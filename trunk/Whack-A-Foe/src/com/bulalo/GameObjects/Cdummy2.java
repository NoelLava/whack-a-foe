...
public class Cdummy2 {
	...
	private Rectangle c2bound;
	private int width;
	private int height;
	private boolean isPressed;
	
	
	public Cdummy2(float x, float y, int width, int height){
		this.width = 22;
		this.height = 42;
		position = new Vector2(20,50);
		
		c2bound = new Rectangle(x, y, width, height);
		
	}
	
	public boolean onClickC2(int screenX, int screenY){
		System.out.println("table on GameScreen changed");
		return c2bound.contains(screenX, screenY);
	}
	
	public boolean isTouchDown(float screenX, float screenY) {
	
        if (c2bound.contains(screenX,screenY)) {
        	System.out.println("table on GameScreen changed");
            String boundS = c2bound.toString();
    ...
    public boolean isTouchUp(int screenX, int screenY) {
        ...
        if (c2bound.contains(screenX, screenY) && isPressed) {
